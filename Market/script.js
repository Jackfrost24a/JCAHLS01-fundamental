...
function handleBuy(sku) {
    ...
    let skudbCart = dbCart.map(val => val.sku)

    if (skudbCart.includes(sku)) {
        let bought = dbCart.findIndex(val => val.sku == sku)
        if (dbCart[bought].qty == dbProduct[index].stock) {
...
function handleDeleteCart(sku) {
    ...
    let DeleteCart = dbCart.findIndex(val => val.sku == sku)

    if (dbCart[DeleteCart].qty == 1) {
        dbCart.splice(DeleteCart, 1)
        printKeranjang()
    } else {
        dbCart[DeleteCart].qty -= 1
        printKeranjang()
